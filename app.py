...
def show_notion_import_page():
    ...
    st.markdown("ローカル版でエクスポートしたCSV、またはNotionの `.md` ファイルを取り込めます。")

    tab_csv, tab_md = st.tabs(["📊 CSVインポート（推奨）", "📄 mdファイルインポート"])

    # ===== CSVインポートタブ =====
    with tab_csv:
        st.markdown("ローカル版アプリの「Notion履歴」画面から「📥 CSVでエクスポート」したファイルをインポートします。")
        ...
        if csv_file:
            ...
            try:
                ...
                st.success(f"✅ {len(csv_rows)}件を読み込みました")

                st.subheader("プレビュー（先頭10件）")
                for r in csv_rows[:10]:
                    c1, c2, c3 = st.columns([3, 1, 3])
                    with c1:
                        st.markdown(f"**{r.get('dish', '')}**")
                    with c2:
                        st.caption(f"🌟 {r.get('count', 1)}回")
                    with c3:
                        st.caption(r.get("dates", ""))
                if len(csv_rows) > 10:
                    st.caption(f"... 他 {len(csv_rows) - 10}件")
                ...
                if st.button("✅ CSVを取り込む", use_container_width=True, type="primary", key="csv_import_btn"):
                    ...
                    for r in csv_rows:
                        ...
                        try:
                            ...
                            count = 1
                        dates_raw = r.get("dates", "")
                        dates = [d.strip() for d in dates_raw.replace(",", "、").split("、") if d.strip()] if dates_raw else []
                        new_records.append({"dish": dish, "count": count, "dates": dates, "recipe": r.get("recipe", "")})
                    by_dish = {r["dish"]: r for r in existing}
                    added, updated = 0, 0
                    for rec in new_records:
                        dish = rec["dish"]
                        if dish in by_dish:
                            by_dish[dish]["count"] = max(by_dish[dish].get("count", 1), rec["count"])
                            existing_dates = set(by_dish[dish].get("dates", []))
                            existing_dates.update(rec.get("dates", []))
                            by_dish[dish]["dates"] = sorted(existing_dates)
                            if rec.get("recipe"):
                                by_dish[dish]["recipe"] = rec["recipe"]
                            updated += 1
                        else:
    ...
    # ===== mdインポートタブ =====
    with tab_md:
        st.markdown("Notionからエクスポートした `.md` ファイルをアップロードします。処理に時間がかかる場合があります。")
